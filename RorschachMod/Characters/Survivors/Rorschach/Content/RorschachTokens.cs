...
namespace RorschachMod.Characters.Survivors.Rorschach
{
    public static class RorschachTokens
    {
        ...
        public static void AddRorschachTokens()
        {
            ...
            Language.Add(prefix + "SPECIAL_ON_KILL_BUFF_KEYWORD", $"<style=cKeywordName>{specialOnKillBuffName}</style><style=cSub>Increases attack speed and movement speed by {specialOnKillBuffMultiplier*100f}% and regen by {specialOnKillBuffRegenMultAdd * 100f}% for {specialOnKillBuffDuration}s. Judgement increases this duration by {specialOnKillBuffDurationPerJudgement}s per stack.</style>");

            Language.Add(prefix + "SPECIAL_FLAME_CAN_NAME", $"Homemade Explosive");
            Language.Add(prefix + "SPECIAL_FLAME_CAN_DESCRIPTION", $"{Tokens.DamageText($"{specialFlameCanFinalBurnStacks}xIgnite")}. Prime your flame can to explode before throwing it forward, dealing {Tokens.DamageValueText(specialFlameCanDamageCoefficient)}. Kills grant {Tokens.UtilityText(specialOnKillBuffName)}. {Tokens.RedText("Judgement")} gives {Tokens.DamageText($"{specialFlameCanJudgementDamageMultiplier*100f}% increased damage")} and {Tokens.UtilityText($"{specialFlameCanJudgementExplosionRadiusMultiplier * 100f}% increased explosion radius")} per stack. Using this move {Tokens.RedText("consumes")} your {Tokens.DamageText("flame can")}.");
