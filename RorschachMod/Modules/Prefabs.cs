...
namespace RorschachMod.Modules
{
    ...
    internal static class Prefabs
    {
        ...
        public static GameObject CreateDisplayPrefab(string displayPrefabGUID, GameObject bodyPrefab)
        {
            GameObject display = Addressables.LoadAssetAsync<GameObject>(displayPrefabGUID).WaitForCompletion();
            if (display == null)
            {
                Log.Error($"could not load display prefab {displayPrefabGUID}. Make sure the addressable build has been updated");
                return null;
            }
        ...
        #region body setup
        public static GameObject LoadCharacterModel(string modelGUID)
        {
            GameObject model = Addressables.LoadAssetAsync<GameObject>(modelGUID).WaitForCompletion();
            if (model == null)
            {
                Log.Error($"could not load model prefab {modelGUID}. Make sure the addressable build has been updated");
                return null;
            }
            return model;
        }
    ...
    public class CustomRendererInfo
    {
        ...
        public AssetReferenceT<Material> materialReference = null;
